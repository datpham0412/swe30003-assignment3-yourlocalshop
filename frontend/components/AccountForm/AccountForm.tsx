...
export default function AccountForm({ backHref = "/catalogue", onSaved }: Props) {
    ...
    const fetchAccount = async () => {
      ...
      try {
        const resp = await fetch(
          `http://localhost:5074/api/Auth/me?email=${encodeURIComponent(emailLS)}&password=${encodeURIComponent(passwordLS)}`,
        )
        if (!resp.ok) {
          try {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const resp = await fetch(
        `http://localhost:5074/api/Auth/update?email=${encodeURIComponent(emailLS)}&password=${encodeURIComponent(passwordLS)}`,
        {
          method: "POST",
          ...
          <CardContent>
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">Changing email will be checked for uniqueness before allowing the update.</p>
              </div>
              ...
              <div className="flex items-center justify-between">
                <Button type="submit" className="bg-gradient-to-r from-purple-600 to-purple-700" disabled={saving}>
                  {saving ? (
